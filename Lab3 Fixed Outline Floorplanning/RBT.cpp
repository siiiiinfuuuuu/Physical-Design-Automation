...
void RBT::DeleteRBT(int KEY){              // 要刪除具有KEY的node
    ...
    if (y != delete_node) {                // 針對case3
        ...
        delete_node->value = y->value; // 放回delete_node的記憶體位置, 並將y的記憶體位置釋放
    }
...
TreeNode* RBT::InsertRBT(int key, int value){
    // 前半部與 InsertBST()之邏輯完全相同, 僅僅需要修改 NULL <-> NIL
    TreeNode *y = neel;
    TreeNode *x = root;
    TreeNode *insert_node = new TreeNode(key, value);
...
TreeNode* RBT::Successor_2(TreeNode *current){
